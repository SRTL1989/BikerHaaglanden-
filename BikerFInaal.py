...
Ontwikkeld in opdracht van: De Haagse Hogeschool LU3
...
class BikerRentalApp:
    ...
    def create_widgets(self):
        ...
        # Hoofdcontainer met ruime padding (Negatieve ruimte)
        container = ttk.Frame(self.root, padding="30", style='TFrame')
        container.pack(fill=tk.BOTH, expand=True)
        container.columnconfigure(0, weight=3)  # Linkerkolom claimt de ruimte (asymmetrie)
        ...
        # Stadsfietsen rij
        ttk.Label(grid_fietsen, text="Stadsfiets Dames:", style='THHSR.TLabel').grid(row=0, column=0, sticky=tk.W,
                                                                                     pady=5, padx=(0, 10))
        ...
        ttk.Label(grid_fietsen, text="Stadsfiets Heren:", style='THHSR.TLabel').grid(row=0, column=2, sticky=tk.W,
                                                                                     pady=5, padx=(0, 10))
        ...
        # E-bikes rij
        ttk.Label(grid_fietsen, text="Elektrische Dames:", style='THHSR.TLabel').grid(row=1, column=0, sticky=tk.W,
                                                                                      pady=5, padx=(0, 10))
        ...
        ttk.Label(grid_fietsen, text="Elektrische Heren:", style='THHSR.TLabel').grid(row=1, column=2, sticky=tk.W,
                                                                                      pady=5, padx=(0, 10))
        ...
        # Sectie: Blokken voor Accessoires & Periode
        ...
        ttk.Checkbutton(acc_frame, text="Helm (€3.00/dg)", variable=self.helm_var, command=self.update_prijs,
                        style='THHSR_Check.TCheckbutton').pack(anchor=tk.W, pady=2)
        self.kinderzitje_var = tk.BooleanVar()
        ttk.Checkbutton(acc_frame, text="Kinderzitje (€4.00/dg)", variable=self.kinderzitje_var,
                        command=self.update_prijs, style='THHSR_Check.TCheckbutton').pack(anchor=tk.W, pady=2)
        ...
        # Business rule: Kortingsstructuur
        ...
        ttk.Checkbutton(main_frame, text="Terugkerende klant / THHSR Student (10% korting)",
                        ...
                        style='THHSR_Check.TCheckbutton').pack(anchor=tk.W, pady=(10, 15))
        ...
        ttk.Radiobutton(pay_frame, text="iDEAL (Online)", variable=self.betaalmethode_var, value='ideal',
                        style='THHSR_Radio.TRadiobutton').pack(side=tk.LEFT, padx=(0, 20))
        ttk.Radiobutton(pay_frame, text="PIN (Bij afhalen)", variable=self.betaalmethode_var, value='pin',
                        style='THHSR_Radio.TRadiobutton').pack(side=tk.LEFT)

        # Actieknoppen (Minimalistisch, plat design)
        ...
        tk.Label(sidebar_frame, text="TOTAALPRIJS", font=('Source Sans Pro', 14, 'bold'), fg='#26384A',
                 bg='#A3AD00').pack(anchor=tk.W, pady=(0, 10))

        # Enorme typografische focus op het eindbedrag
        ...
        # Transparant wit detailvenster voor een geordend overzicht
    ...
    def bereken_prijs(self):
        """Calculatie-engine: berekent de huurprijs, kortingen en cumulatieve borg"""
        try:
    ...
    def update_prijs(self):
        """Synchroniseert de UI-labels met de calculatie-engine"""
        totaal_met_borg, details, dagen, borg_bedrag, verhuur_totaal, totaal_fietsen = self.bereken_prijs()
    ...
    def reserveer(self):
        """Valideert invoergegevens en committeert de data naar de CSV-laag"""
        ...
        totaal_met_borg, _, dagen, borg_bedrag, verhuur_totaal, totaal_fietsen = self.bereken_prijs()
        if totaal_fietsen == 0:
        ...
        try:
            ...
            messagebox.showerror("Fout", f"Transactie afgebroken, database error: {e}")
            return

        bevestiging_tekst = f"Succesvol Gereserveerd!\n\nID: {reserverings_id}\nNaam: {naam}\nAantal fietsen: {int(totaal_fietsen)}\nPeriode: {start_str} tot {eind_str}\nTotaalbedrag (incl. borg): €{totaal_met_borg:.2f}"
        messagebox.showinfo("Reservering Bevestigd", bevestiging_tekst)
        self.reset_form()

    def reset_form(self):
        """Zet alle formuliervariabelen terug naar de standaard beginwaarden"""
        self.naam_var.set('')
        self.email_var.set('')
    ...
    def toon_reserveringen(self):
        """Genereert het administratieve data-overzichtsscherm"""
        if not os.path.exists(self.csv_bestand):
            messagebox.showinfo("Info", "Geen historische data gevonden.")
            return
        ...
        try:
            ...
            messagebox.showerror("Fout", f"Leesfout in database: {e}")
            window.destroy()
        ...
        def annuleer_reservering():
            """Business logica: Annuleringsprotocol met de 24-uurs harde restrictie"""
            selection = tree.selection()
            if not selection:
                messagebox.showwarning("Waarschuwing", "Selecteer een record")
                return
            ...
            if values[18] == 'Geannuleerd':
                messagebox.showinfo("Info", "Dit record is reeds geannuleerd.")
                return

            reserverings_id = values[0]
            startdatum_str = values[10]

            try:
                startdatum = datetime.strptime(startdatum_str, "%d-%m-%Y")
            except:
                messagebox.showerror("Fout", "Corrupte datumstructuur gedetecteerd.")
                return

            # Validatie van de 24-uurs termijn
            if (startdatum - datetime.now()).total_seconds() / 3600 < 24:
                messagebox.showerror("Annulering Geweigerd",
                                     "Annuleringen dienen minimaal 24 uur voor aanvang plaats te vinden.")
                return

            if not messagebox.askyesno("Bevestig", f"Weet u zeker dat u {reserverings_id} wilt annuleren?"):
                return
            ...
            try:
                ...
                messagebox.showerror("Fout", f"Mutatie mislukt: {e}")
...
def main():
    ...
    try:
        ...
        pass
    # Start de applicatie-lifecycle via de Login-omgeving
    LoginWindow(root)
    root.mainloop()
